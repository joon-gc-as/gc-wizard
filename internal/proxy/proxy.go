...
func forwardEvent(ctx context.Context, client *http.Client, targetURL string, data []byte) error {
	...
	if err := json.Unmarshal(data, &fields); err != nil {
		return fmt.Errorf("decode proxy event: %w", err)
	}
...
func streamOnce(ctx context.Context, client *http.Client, proxyURL, targetURL string) error {
	...
	log.Printf("proxy: connected to %s, forwarding GitHub events to %s", proxyURL, targetURL)
	...
	if err := scanner.Err(); err != nil {
		return fmt.Errorf("read proxy stream: %w", err)
	}
	return fmt.Errorf("proxy stream closed")
}

func ForwardGHEventsToLocalApp(ctx context.Context, proxyURL string) {
	targetURL := fmt.Sprintf("http://localhost:%s/webhooks/github", os.Getenv("APP_PORT"))
	client := &http.Client{}

	for ctx.Err() == nil {
		if err := streamOnce(ctx, client, proxyURL, targetURL); err != nil && ctx.Err() == nil {
			log.Printf("proxy: proxy stream error: %v", err)
		}
